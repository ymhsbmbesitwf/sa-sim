...
const builder = {
    ...
    addOtherCosts: function () {
        for (let oneTimerId in sim.oneTimers) {
            ...
            if (oneTimer.owned) {
                builder.costDust +=
                    builder.costUnlock[oneTimerId] * !oneTimer.useShards;
                builder.costShard +=
                    builder.costUnlock[oneTimerId] * !!oneTimer.useShards;
            }
        }
        builder.costShard +=
            builder.costCurrent.The_Ring * sim.oneTimers.The_Ring.owned;
        builder.costDust +=
            builder.limbs > 4
                ? (100 * (Math.pow(100, builder.limbs - 4) - 1)) / 99
                : 0;
    },
    costCurrent: {},
    costDust: 0,
    ...
    modifiers: {
        ...
        Eelimp_in_a_Bottle: function () {
            ...
            builder.huffy.shockMin += chance;
            builder.huffy.shockMax += chance;
            builder.huffy.resistShock +=
                sim.items.Eelimp_in_a_Bottle.shockResist();
        },
        ...
        Aegis: function () {
            ...
            if (
                sim.items.Basket_of_Souls.equipped ||
                sim.items.Nullifium_Armor.equipped
            ) {
                builder.huffy.shockMin += chance;
    ...
    readEnemy: function () {
        ...
        builder.enemy.bleed = s
            ? Math.ceil(
                  sim.enemyLevel *
                      3 *
                      (2 - Math.pow(0.5, (s[2] ? parseInt(s[2]) : 1) - 1))
              )
            : 0;
        s = /Poisoning(| x(\d))(?=(,|$))/.exec(profile);
        builder.enemy.poison = s
            ? Math.ceil(
                  sim.enemyLevel *
                      3 *
                      (2 - Math.pow(0.5, (s[2] ? parseInt(s[2]) : 1) - 1))
        ...
        if (s <= 0) {
            ...
            builder.enemy.shanked =
                builder.enemy.poison == s
                    ? "poison"
                    : builder.enemy.bleed == s
                    ? "bleed"
                    : "shock";
            document.getElementById("enemyShank").innerHTML =
                builder.enemy.shanked;
        }
        builder.enemy.resistBleed =
            sim.enemyLevel * (/Bleed Resistant/.exec(profile) ? 11 : 1);
        builder.enemy.resistPoison =
            sim.enemyLevel * (/Poison Resistant/.exec(profile) ? 11 : 1);
        builder.enemy.resistShock =
            sim.enemyLevel * (/Shock Resistant/.exec(profile) ? 11 : 1);
    },
    readEquips: function () {
        builder.resetHuffy();
        builder.limbs = 0;
        ...
        sim.bonuses["Extra_Limbs"].level =
            builder.limbs >= 4 ? builder.limbs - 4 : 0;
    ...
    resetHuffy: function () {
        for (let index in builder.huffy) {
            builder.huffy[index] =
                typeof builder.huffy[index] == "boolean" ? false : 0;
        }
    },
    setEnemyLevel: function (level, forceRead = false) {
        if (forceRead || sim.enemyLevel != level) {
            sim.enemyLevel = level;
            document.getElementById("effects").innerHTML = sim.getEffects(
                sim.enemyLevel
            );
            builder.readEnemy();
    ...
    shank: function () {
        ...
        for (let bleed of [builder.huffy.bleedMin, builder.huffy.bleedMax]) {
            for (let poison of [
                builder.huffy.poisonMin,
                builder.huffy.poisonMax,
            ]) {
                for (let shock of [
                    builder.huffy.shockMin,
                    builder.huffy.shockMax,
                ]) {
                    let max = Math.max(bleed, poison, shock);
                    let reduction = 0.25 * max;
                    builder.shankInfo.reductionMin = Math.min(
                        builder.shankInfo.reductionMin,
                        reduction
                    );
                    builder.shankInfo.reductionMax = Math.max(
                        reduction,
                        builder.shankInfo.reductionMax
                    );
        ...
        builder.shankInfo.poison[1] = shankedPoison
            ? builder.shankInfo.poison[1]
            : builder.huffy.poisonMax;
        builder.shankInfo.bleed[1] = shankedBleed
            ? builder.shankInfo.bleed[1]
            : builder.huffy.bleedMax;
        builder.shankInfo.shock[1] = shankedShock
            ? builder.shankInfo.shock[1]
            : builder.huffy.shockMax;
        document.getElementById("huffyShank").innerHTML =
            (shankedPoison
                ? "poison" + (shankedBleed || shankedShock ? "/" : "")
                : "") +
            (shankedBleed ? "bleed" + (shankedShock ? "/" : "") : "") +
            (shankedShock ? "shock" : "");
    ...
    toggleRing: function () {
        if (sim.oneTimers.The_Ring.owned) {
            ...
            sim.rings.level = parseInt(
                document.getElementById("The_Ring_Input").value
            );
            builder.updateRingCost();
            if (sim.rings.level >= 10) {
                builder.readEquips();
    ...
    updateCostDisplay: function () {
        document.getElementById("buildCostDust").innerHTML = scientificCost(
            builder.costDust
        );
        document.getElementById("buildCostShards").innerHTML = scientificCost(
            builder.costShard
        );
    },
    updateDisplay: function () {
        document.getElementById("enemyResistPoison").innerHTML =
            builder.enemy.resistPoison + "%";
        ...
        document.getElementById("enemyResistBleed").innerHTML =
            resistBleed + "%";
        document.getElementById("enemyResistShock").innerHTML =
            resistShock + "%";
        if (sim.items.Sacrificial_Shank.equipped) {
            let resistAllMax =
                builder.huffy.resistAll +
                Math.floor(
                    (builder.shankInfo.reductionMax + builder.enemy.shank) / 10
                ) *
                    sim.items.Sacrificial_Shank.level;
            let resistAllMin =
                builder.huffy.resistAll +
                Math.floor(
                    (builder.shankInfo.reductionMin + builder.enemy.shank) / 10
                ) *
                    sim.items.Sacrificial_Shank.level;
            document.getElementById("enemyPoison").innerHTML =
                builder.enemy.poison -
                resistAllMax -
                builder.huffy.resistPoison -
                (builder.enemy.shanked == "poison" ? builder.enemy.shank : 0) +
                "%" +
                (resistAllMax == resistAllMin
                    ? ""
                    : " to " +
                      (builder.enemy.poison -
                          resistAllMin -
                          builder.huffy.resistPoison -
                          (builder.enemy.shanked == "poison"
                              ? builder.enemy.shank
                              : 0)) +
                      "%");
            document.getElementById("enemyBleed").innerHTML =
                builder.enemy.bleed -
                resistAllMax -
                builder.huffy.resistBleed -
                (builder.enemy.shanked == "bleed" ? builder.enemy.shank : 0) +
                "%" +
                (resistAllMax == resistAllMin
                    ? ""
                    : " to " +
                      (builder.enemy.bleed -
                          resistAllMin -
                          builder.huffy.resistBleed -
                          (builder.enemy.shanked == "bleed"
                              ? builder.enemy.shank
                              : 0)) +
                      "%");
            document.getElementById("enemyShock").innerHTML =
                builder.enemy.shock -
                resistAllMax -
                builder.huffy.resistShock -
                (builder.enemy.shanked == "shock" ? builder.enemy.shank : 0) +
                "%" +
                (resistAllMax == resistAllMin
                    ? ""
                    : " to " +
                      (builder.enemy.shock -
                          resistAllMin -
                          builder.huffy.resistShock -
                          (builder.enemy.shanked == "shock"
                              ? builder.enemy.shank
                              : 0)) +
                      "%");
            document.getElementById("huffyPoison").innerHTML =
                builder.shankInfo.poison[0] == builder.shankInfo.poison[1]
                    ? builder.shankInfo.poison[1] -
                      builder.enemy.resistPoison +
                      "%"
                    : builder.shankInfo.poison[0] -
                      builder.enemy.resistPoison +
                      "% to " +
                      (builder.shankInfo.poison[1] -
                          builder.enemy.resistPoison) +
                      "%";
            document.getElementById("huffyBleed").innerHTML =
                builder.shankInfo.bleed[0] == builder.shankInfo.bleed[1]
                    ? builder.shankInfo.bleed[1] - resistBleed + "%"
                    : builder.shankInfo.bleed[0] -
                      resistBleed +
                      "% to " +
                      (builder.shankInfo.bleed[1] - resistBleed) +
                      "%";
            document.getElementById("huffyShock").innerHTML =
                builder.shankInfo.shock[0] == builder.shankInfo.shock[1]
                    ? builder.shankInfo.shock[1] - resistShock + "%"
                    : builder.shankInfo.shock[0] -
                      resistShock +
                      "% to " +
                      (builder.shankInfo.shock[1] - resistShock) +
                      "%";
            ...
            document.getElementById("enemyShock").innerHTML =
                ...
                "%";
            document.getElementById("huffyPoison").innerHTML =
                builder.huffy.poisonMax == builder.huffy.poisonMin
                    ? builder.huffy.poisonMax - builder.enemy.resistPoison + "%"
                    : builder.huffy.poisonMin -
                      builder.enemy.resistPoison +
                      "% to " +
                      (builder.huffy.poisonMax - builder.enemy.resistPoison) +
                      "%";
            document.getElementById("huffyBleed").innerHTML =
                builder.huffy.bleedMax == builder.huffy.bleedMin
                    ? builder.huffy.bleedMax - resistBleed + "%"
                    : builder.huffy.bleedMin -
                      resistBleed +
                      "% to " +
                      (builder.huffy.bleedMax - resistBleed) +
                      "%";
            document.getElementById("huffyShock").innerHTML =
                builder.huffy.shockMax == builder.huffy.shockMin
                    ? builder.huffy.shockMax - resistShock + "%"
                    : builder.huffy.shockMin -
                      resistShock +
                      "% to " +
                      (builder.huffy.shockMax - resistShock) +
    ...
    updateItemCost: function (itemId) {
        ...
        builder.costCurrent[itemId] =
            builder.costUnlock[itemId] +
            ((item.startPrice || 5) *
                (1 - Math.pow(item.priceMod || 3, item.level - 1))) /
                (1 - (item.priceMod || 3));
    },
    updateRingCost: function () {
        builder.costCurrent.The_Ring = Math.ceil(
            15 * Math.pow(2, sim.rings.level) - 30
        );
    },
    visible: function (id, flag) {
        document
            .getElementById(id)
            .setAttribute("class", flag ? "" : "displayNone");
    },
};
...
for (let itemId in sim.items) {
    builder.costUnlock[itemId] = sim.items[itemId].zone
        ? sim.contractPrice(itemId)
        : 0;
